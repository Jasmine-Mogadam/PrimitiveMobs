...
public class EntityAIFilchSteal extends EntityAIBase {
    ...
    public EntityAIFilchSteal(EntityCreature temptedEntityIn, double speedIn, Set<ItemStack> temptItemIn, boolean canGetScared)
    {
        ...
        if (this.temptedEntity.getDistanceSq(this.temptingPlayer) < 3.25D)
        {
            ...
            for(int i = 0 ; i < this.temptingPlayer.inventory.getSizeInventory() ; i++)
			{
				...
				if(!item.isEmpty())
				{
		    		...
		    		{
		    			if(itemstack != null && !itemstack.isEmpty() && itemstack.getItem() == item.getItem() && itemstack.getMetadata() == item.getMetadata() && !temptingPlayer.capabilities.isCreativeMode)
		    			{
                            ...
                            // Fixed bug: Removed the extra item.shrink(1) call which caused 2 items to be consumed.
                            // splitStack(1) already decrements the stack size.
